...
const mostrando_bonos = () => {
  ...
  setInterval(()=>{ 
    ...
    if(c3){
      ...
      document.getElementById('id_bns').style.border = '5px solid #54a858'
      document.getElementById('mnt_bns').style.border = '5px solid #54a858'
...
setInterval( async ()=> {
  ...
  if( seg_intro.includes( (minutos * 60) + segundos ) && vd && entra_intro){
    ...
    if(nup2[1] == 'X2' || nup2[1] == 'X3') video_intro.src = `static/videos/dog8/intro${nup2[1]}.mp4`;
    else                                   video_intro.src = `static/videos/dog8/intro.mp4`;  
