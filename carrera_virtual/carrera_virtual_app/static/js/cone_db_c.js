...
const Consultas_jackpot_carrera = async () =>{
    ...
    try {
        ...
        const response = await fetch("/carreras_virtual_c",{ method:"POST", body:JSON.stringify(datos_re), headers:{"X-CSRFToken":getCookie2('csrftoken'), "X-Requested-With":"XMLHttpRequest", 'Content-Type':'application/json'}})
...
const Consulta_bonos = async () => {
    ...
    try{
        ...
        const datos_re = {'realizar':'consulta_bonos', "id_lugar" : lugar, "juego": 'CABALLOS_7'};
...
const Consulta_Tabla = async () => {
    ...
    try {
        ...
        const datos_re = {'realizar':'consulta_tabla','juego':'CABALLOS_7', "id_grupo" : localStorage.getItem('grp')};

        var response = await fetch("/carreras_virtual_c",{ method:"POST", body:JSON.stringify(datos_re), headers:{ "X-CSRFToken":getCookie2('csrftoken'), "X-Requested-With":"XMLHttpRequest", 'Content-Type':'application/json'}})
        ...
        while(good && cnt < 10){
            ...
            cnt +=1                 
            response = await fetch("/carreras_virtual_c",{ method:"POST", body:JSON.stringify(datos_re), headers:{ "X-CSRFToken":getCookie2('csrftoken'), "X-Requested-With":"XMLHttpRequest", 'Content-Type':'application/json'}})
...
const Consulta_resultados = async () => {
    ...
    const datos_re = {'realizar':'consulta_resultados','juego':'CABALLOS_7', "id_grupo" : localStorage.getItem('grp')};
    const response = await fetch("/carreras_virtual_c",{ method:"POST", body:JSON.stringify(datos_re), headers:{"X-CSRFToken":getCookie2('csrftoken'), "X-Requested-With":"XMLHttpRequest", 'Content-Type':'application/json'}})
    const data     = await response.json() 
    ...
    document.getElementById("img_win").src = `static/img/numeros/h7/n${pos1}.svg`;
    document.getElementById("p_win").innerHTML = Number(pago_win)
   
    document.getElementById("img1_ext").src = `static/img/numeros/h7/n${pos1}.svg`;
    document.getElementById("img2_ext").src = `static/img/numeros/h7/n${pos2}.svg`;
...
const Consulta_ultimas_carreras = async () => {
    ...
    try{
        ...
        const datos_re = {'realizar':'consulta_ult_carreras','juego':'CABALLOS_7', "id_grupo" : localStorage.getItem('grp')};
        ...
        data.map((races, cont)=>{
           ...
           document.getElementById(`lugarimg_1er${cont}`).src =  `static/img/numeros/h7/n${races[0]['race_winner'].substr(0,1)}.svg`
           document.getElementById(`lugarprc_1er${cont}`).innerHTML = Number(races[0]['pago_primer_lugar']) 
           
           
           document.getElementById(`lugarimg_pls_1er${cont}`).src =  `static/img/numeros/h7/n${races[0]['race_winner'].substr(0,1)}.svg`
           document.getElementById(`lugarimg_pls_2do${cont}`).src =  `static/img/numeros/h7/n${races[0]['race_winner'].substr(2,1)}.svg`
